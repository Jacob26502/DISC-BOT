...
class DiscordBot():
    ...
    async def on_message(message):
        ...
        #print(int(db.get(person.id == str(message.author.id)).get('msgcount')))
        ...
        if nekomode == True:
            ...
            buh = re.sub("n([aeiou])", "ny\\1",
                         re.sub("[lr]", "w", re.sub("v", "ff", message.clean_content, flags=re.IGNORECASE),
    ...
    @bot.command(name='server%', pass_context=True, brief='tells you percentage of messages compared to server total',
                 aliases=['sad'], usage="Input a mention, an ID or 'self'")
    ...
    @bot.command(name='leadercompare', pass_context=True, brief='tells you percentage of messages compared to leader',
                 aliases=['lcomp'], usage="Input a mention, an ID or 'self'")
    async def depression(ctx, user: typing.Optional[discord.Member] = None, *args):
        try:
            if args[0] == "self":
                user = ctx.message.author
